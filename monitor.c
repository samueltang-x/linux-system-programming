...
void main() {
  ...
  while (fgets(watchname, PATH_MAX, fconfig) != NULL) {
    ...
    if (S_ISREG(sb.st_mode)) {
      ...
      if ((watchfd = inotify_add_watch(notifyfd, watchname, IN_MODIFY | IN_DELETE_SELF)) < 0) {
        ...
        printf("added %s to watch list on descripter %d\n", watchname, watchfd);
  ...
  /* All our wtachs are in place, so just read and report events */
  fout = fopen("monitor.out", "a");

  while(1) {
    n = read(notifyfd, eventbuf, BUFSIZE);
    /* Loop over all events and report them. This is litter tricky
